...
int switch_func(int scelta, int *pn, corsa **pcnt);
...
int isDataGr(corsa *c1, corsa *c2);
...
int main(){
    ...
    fscanf(fp_read, "%d", &n);
    corsa *cnt = (corsa*) malloc(n*sizeof(corsa));
...
void stampa_tratte_p(corsa **vpp, int n){
        for(int i=0; i<n; i++){
            printf("%s %s %s ", vpp[i] -> cod_tratta, vpp[i] -> staz_part, vpp[i] -> staz_arr);
            printf("%d/%.2d/%.2d ", vpp[i] -> data_part.yyyy, vpp[i] -> data_part.mm, vpp[i] -> data_part.dd);
            printf("%.2d:%.2d:%d ", vpp[i] -> orario_part.ora, vpp[i] -> orario_part.min, vpp[i] -> orario_part.sec);
            printf("%.2d:%.2d:%d ", vpp[i] -> orario_arr.ora, vpp[i] -> orario_arr.min, vpp[i] -> orario_arr.sec);
            printf("%d\n", vpp[i] -> ritardo);
        }
}

int isDataGr(corsa *c1, corsa *c2){
    if(c1 -> data_part.yyyy < c2 -> data_part.yyyy) return -1;
        else if(c1 -> data_part.yyyy == c2 -> data_part.yyyy && c1 -> data_part.mm < c2 -> data_part.mm) return -1;
        else if(c1 -> data_part.yyyy == c2 -> data_part.yyyy && c1 -> data_part.mm == c2 -> data_part.mm && c1 -> data_part.dd > c2 -> data_part.dd){
            return 1;
        }else if (c1 -> data_part.yyyy == c2 -> data_part.yyyy && c1 -> data_part.mm == c2 -> data_part.mm && c1 -> data_part.dd == c2 -> data_part.dd){
            if(c1 -> orario_part.ora < c2 -> orario_part.ora) return -1;
            else if(c1 -> orario_part.ora == c2 -> orario_part.ora && c1 -> orario_part.min < c2 -> orario_part.min) return -1;
...
int switch_func(int scelta, int *pn, corsa **pcnt){
    ...
    switch(scelta){
        case 1:
            ...
            else if(tolower(chr)=='f'){if(stampa_tratte(cnt, n, 1) < 0) {free(vet); return -1;}}
            else {free(vet); return -1;}
            break;
        case 2:
            copy_ad(cnt, vet, n);
        ...
        case 8:
            ...
            if((f_in = fopen(nfile, "r")) == NULL){
                printf("Errore durante l'apertura del file");
                return -1;
            }
            fscanf(f_in, "%d", pn);
            cnt = (corsa*)realloc(*pcnt, *pn*sizeof(corsa));
            leggi_tratta(f_in, *pn, cnt);
            fclose(f_in);
            *pcnt = cnt;
            break;
    }
...
int core(FILE *fin, int *n, corsa *contenitore){
    ...
    while(1==1){
        ...
        printf("\t8)Acquisisci nuovi dati via file\n");

        printf("Scegli: ");
        scanf("%d", &scelta);
        if(scelta < 0 || scelta > 8) return -1;
